...
int getActionType(LoqState masked_state, LoqState previous){
    ...
    if (!masked_state.get_turn()){
        ...
        if (type == 2){
            ...
            for (int i=0; i<7; ++i){
                if (currentiblock[6-i].dir != 0){
                    idx = 6-i;
                    break;
                }
            }
            if (currentiblock[idx].dir != 5) return -2;
            return 2;
        }
        else if (type == 3){
            int idx = -1; // 제일 마지막에 둔 블럭 idx
            for (int i=0; i<3; ++i){
                if (currentlblock[2-i].dir != 0){
                    idx = 2-i;
                    break;
                }
            }
            if (currentlblock[idx].dir != 5) return -3;
            return 3;
        }
        else{
            if (masked_state.get_internal_state().position(false).x() != 0) return -1;
            return 1;
        }
    }
    else {
    ...
    if (type == -1){ // 상대가 움직였는데 부쉬 밖에 있을 때
        ...
        if (masked_state.get_internal_state().position(false).x() == 0 && !previous.get_internal_state().internal(true)){
            ...
            if (valid_actions.empty()){
                ...
                action = valid_actions[(rand()/10)%valid_actions.size()];
...
LoqState makePredictedState(LoqState masked_state, LoqState& predicted_state, MCTS::Action* prev_action){
    ...
    if (masked_state.get_internal_state().internal(true)){
        ...
        if (prev_action != nullptr){
            ...
            LoqState* st_ptr = correctState(masked_state, predicted_state, prev_action);
            predicted_state = *st_ptr; // turn은 상대턴임
            predicted_state = *predictState(masked_state, predicted_state);
        }
        else{
            if (masked_state.get_turn() != predicted_state.get_turn()){
                predicted_state = masked_state;
            }
        }
    }
